...
class App < Sinatra::Base
  ...
  get '/message' do
    ...
    statement = db.prepare('SELECT mes.*, user.name, user.display_name, user.avatar_icon FROM message as mes LEFT JOIN user AS user ON mes.user_id = user.id WHERE mes.id > ? AND mes.channel_id = ? ORDER BY id DESC LIMIT 100')
